...
const statusPill = (status: ApplicationStatus) => {
  ...
  return (
    <span className={`inline-flex items-center gap-1.5 border px-2 py-0.5 font-mono text-[10px] uppercase tracking-wide ${option.border} ${option.bg} ${option.text}`}>
...
export default function CareerApplicationsPage() {
  ...
  useEffect(() => {
    if (careerId) fetchApplications();
  }, [careerId, statusFilter]);

  const fetchApplications = async (page: number = 1) => {
    try {
      setLoading(true);
      setError(null);
      const response = await getApplicationsByCareer(
        careerId,
        page,
        20,
      ...
      if (response.career) setCareer(response.career);
    } catch (err: any) {
  ...
  const handleStatusChange = async (
    ...
    try {
      ...
      setApplications((prev) =>
        prev.map((a) =>
          a.id === app.id
            ? { ...a, attributes: { ...a.attributes, status: newStatus } }
            : a
        )
      );
      if (meta?.status_counts) {
        const old = app.attributes.status;
        setMeta({
          ...meta,
          status_counts: {
            ...meta.status_counts,
            [old]: Math.max(0, (meta.status_counts[old] || 0) - 1),
            [newStatus]: (meta.status_counts[newStatus] || 0) + 1,
          },
        });
      }
    } catch (err: any) {
      ...
      <div className="border-b border-[#241d18]/15 bg-[#fffaf1]">
        <div className="mx-auto max-w-7xl px-6 py-8">
          ...
          <div className="mb-2 flex w-fit items-center gap-2 border border-[#241d18]/15 bg-[#f4efe4] px-3 py-1.5 font-mono text-[11px] uppercase tracking-wide text-[#6f665d]">
            ...
            {career ? `${career.department} • ${career.location}` : "Job Applications"}
          </div>
          <h1 className="font-serif text-5xl leading-[1.05] text-[#241d18]">
            {career ? career.title : "Applications"}
          </h1>
        ...
        <div className="mb-6 flex items-end justify-between">
          ...
          >
            ...
            <SelectContent className="rounded-none border-[#241d18]/20">
              <SelectItem value="all" className="text-[#241d18] focus:bg-[#f4efe4]">
                All Statuses
              </SelectItem>
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-1.5">
                    <span
                      className={`size-2 ${active ? "bg-[#fffaf1]" : option.dot}`}
                    />
                    <span
        ...
        <div className="border border-[#241d18]/15 bg-[#fffaf1] shadow-[10px_10px_0_#241d18]">
          ...
          {applications.length === 0 ? (
            <div className="flex flex-col items-center justify-center px-5 py-16">
              ...
              <p className="font-mono text-xs uppercase text-[#6f665d]">No applications yet</p>
            </div>
          ) : (
            applications.map((app) => (
              ...
              >
                ...
                <div className="truncate pr-4">
                  {(() => {
                    const c =
                      typeof app.relationships?.career?.data?.id === "object"
                        ? app.relationships?.career?.data?.id
                        : null;
                    return (
                  ...
                  <button
                    onClick={(e) => {
                      ...
                      const next = STATUS_OPTIONS.find(
                        (s) => s.value !== app.attributes.status
                      );
                      if (next) handleStatusChange(app, next.value);
                    }}
